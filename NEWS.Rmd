...
-   `remove_collisions()` now resolves collisions by first comparing relative
    integration-site abundance across independent samples. Abundance is measured
    with the sequence count column selected through `quant_cols["seqCount"]`,
    summed within each independent sample.
-   Observations at least `fold_threshold` times less abundant than the maximum
    abundance observed for the same integration site are removed before any
    temporal rule is applied. The default threshold is 10 and the exact
    threshold is inclusive (`max(seqCount) / seqCount >= fold_threshold`).
-   The temporal rule is now applied only to collision observations not resolved
    by the fold-abundance comparison.
...
-   Fixed bug in call to `print.data.table` that broke examples for `CIS_Grubbs`.

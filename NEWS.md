...
- After fold-abundance filtering, `remove_collisions()` removes an integration
  site from each independent sample represented by only one row and retains it
  in every sample represented by at least two rows. The same integration site
  may therefore remain in multiple patients.
- Sequencing date is no longer used to resolve collisions. `date_col` is kept
  as an ignored argument for backward compatibility, and `ReplicateNumber` is
  no longer required.
...
- `remove_collisions()` now resolves collisions by first comparing
  relative integration-site abundance across independent samples.
  Abundance is measured with the sequence count column selected through
  `quant_cols["seqCount"]`, summed within each independent sample.
- Observations at least `fold_threshold` times less abundant than the
  maximum abundance observed for the same integration site are removed
  before any temporal rule is applied. The default threshold is 10 and
  the exact threshold is inclusive
  (`max(seqCount) / seqCount >= fold_threshold`).
- The temporal rule is now applied only to collision observations not
  resolved by the fold-abundance comparison.

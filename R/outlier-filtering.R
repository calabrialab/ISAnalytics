...
outlier_filter <- function(metadata,
    ...
    params_for_report <- list(dyn_vars = list(pcr_id = pcr_id_col),
                              input_stats = list(nrow = nrow(metadata),
                                                 n_samples = length(
                                                   unique(
                                                     metadata[[pcr_id_col]]))))
    mode <- "CALL"
    ...
    if (is.null(outlier_test) & is.null(outlier_test_outputs)) {
      err_msg <- c(paste("One between `outlier_test` and",
    ...
    if (mode == "CALL") {
      ...
      .outlier_test_verify_logiop(outlier_test, combination_logic,
                                  "combination_logic")
    } else {
      .outlier_test_verify_logiop(seq_len(test_num),
                                  combination_logic,
                                  "combination_logic")
    }
    ...
    if (mode == "CALL") {
      ...
      call_test <- function(test, vargs, df, test_name) {
        ...
        .validate_outlier_output_format(result,
      ...
      outlier_test_outputs <- purrr::map2(outlier_test,
                                          names(outlier_test),
                                  ~ call_test(.x, dots, metadata,
                                              .y))
      params_for_report$call_args <- purrr::map(outlier_test_outputs,
                                                ~ .x$call_args)
      outlier_test_outputs <- purrr::map(outlier_test_outputs, ~ .x$result)
    } else {
      purrr::walk(outlier_test_outputs, ~ {
        data.table::setDT(.x)
        .validate_outlier_output_format(.x, unique(metadata[[pcr_id_col]]),
                                        pcr_id_col)
      })
      if (is.null(names(outlier_test_outputs))) {
        outlier_test_outputs <- outlier_test_outputs %>%
          purrr::set_names(paste("test", seq(1, length(outlier_test_outputs)),
                                 sep = "_"))
      }
    }
    params_for_report$test_results <- outlier_test_outputs
    to_rem_cols <- purrr::map(outlier_test_outputs,
    ...
    if (negate) {
      metadata_filtered <- metadata[eval(sym("to_remove")) == TRUE,
                                    !c("to_remove")]
    } else {
      metadata_filtered <- metadata[eval(sym("to_remove")) == FALSE,
                                    !c("to_remove")]
    }
...
outliers_by_pool_fragments <- function(metadata,
    ...
    ## Check
    pcr_id_col <- pcr_id_column()
    .outlier_pool_frag_base_checks(metadata, key, outlier_p_value_threshold,
    ...
    if (getOption("ISAnalytics.reports") == TRUE && !is.null(report_path)) {
        withCallingHandlers(
            {
              af_cols_specs <- data.table::setDT(association_file_columns(TRUE))

...
```{r}
quant_folders_found <- length(params$fs_align$Path_quant[!is.na(params$fs_align$Path_quant)])
valueBox(quant_folders_found, icon = "fa-table")
```

### VISPA2 stats folders found
```{r}
stats_folders_found <- length(params$fs_align$Path_iss[!is.na(params$fs_align$Path_iss)])

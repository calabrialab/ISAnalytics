...
test_that(".messy_to_tidy produces correct tidy ISADataFrame - annotated new", {
  ...
  expect_true(all(c("CompleteAmplificationID",
                    "Value") %in% colnames(tidy_isadf)))
...
test_that("import_single_Vispa2Matrix succeeds with standard annotated", {
  ...
  expect_equal(metadata(isadf), c("GeneName", "GeneStrand",
                                  "CompleteAmplificationID"))
...
test_that(".check_af_correctness returns TRUE for correct file", {
  af <- tibble::as_tibble(read.csv(path_af, check.names = FALSE, sep = "\t",
...
missing_project <- list(ProjectID = "PROJECT1101",
...
test_that(".check_file_system_alignment finds missing projects for incorrect
  ...
  expect_equal(
    (checks %>%
      dplyr::filter(.data$ProjectID == missing_project$ProjectID,
                    .data$concatenatePoolIDSeqRun ==
                      missing_project$concatenatePoolIDSeqRun))$Found, FALSE)
  expect_equal(
    (checks %>%
       dplyr::filter(.data$ProjectID == missing_project$ProjectID,
                     .data$concatenatePoolIDSeqRun ==
                       missing_project$concatenatePoolIDSeqRun))$Path,
    NA_character_)
})
...
test_that(".update_af_after_alignment updates correct fs - no NAs", {
  checks <- .check_file_system_alignment(af_df, root_correct)
  updated_af <- .update_af_after_alignment(af_df, checks = checks,
                                           root = root_correct)
...
warning_update_after_alignment <-
  paste0("One or more projects were not found in the file system ",
         "starting from ", root_err,
         ", please check your association ",
...
test_that("import_association_file imports af with warnings for fserr", {
  ...
  missing <- af %>% dplyr::filter(.data$ProjectID == missing_project$ProjectID,
                                  .data$concatenatePoolIDSeqRun ==
                                    missing_project$concatenatePoolIDSeqRun)
  expect_true(all(is.na(missing$Path)))
})
...
#### OTHER VARS ####
all_proj_pools <- list(ProjectID = c("CLOEXP", "PROJECT1100", "PROJECT1101"),
                       PoolID = c("POOL6-1", "ABX-LR-PL5-POOL14-1",
...
test_that(".interactive_select_projects_import selects only some projects
          when input 1 is '2'", {
            ...
            project_list <- dplyr::distinct(
              dplyr::select(as_file_correct, .data$ProjectID))$ProjectID
            invisible(capture_output({
              updated_af <- .interactive_select_projects_import(as_file_correct)
            }))
...
test_that(".interactive_select_pools_import selects only the first pools
  ...
  available <- as_file_correct %>%
    ...
    dplyr::distinct()
  pools_selected <- purrr::map(unique(available$ProjectID),
                               function(x) {
    pools <- (available %>%
...
#------------------------------------------------------------------------------#
smpl_ff <- function() {
  tb <- tibble::tibble(ProjectID = c("PROJ1", "PROJ1", "PROJ2", "PROJ2",
                                     "PROJ3", "PROJ3"),
                 concatenatePoolIDSeqRun = c("POOL1", "POOL1", "POOL2", "POOL2",
                                             "POOL3", "POOL3"),
                 Quantification_type = c("fragmentEstimate", "seqCount",
                                         "fragmentEstimate", "seqCount",
                                         "fragmentEstimate", "seqCount"),
                 Files_found = c("pth1", "pth2", "pth3", "pth4", "pth5",
                                 "pth6"))
...
test_that(".lookup_matrices finds all files for fs - no duplicates", {
  ...
  expect_true(all(c("ProjectID", "concatenatePoolIDSeqRun", "Anomalies",
                    "Files", "Files_count") %in% colnames(files_found)))
...
test_that(".lookup_matrices finds all files for fserr - duplicates", {
  ...
  cloexp_anomalies <- (files_found %>%
    dplyr::filter(.data$ProjectID == "CLOEXP",
                  .data$concatenatePoolIDSeqRun == "POOL6-1"))$Files_count[[1]]
  expect_equal(dplyr::filter(
    cloexp_anomalies, .data$Quantification_type == "fragmentEstimate")$Found, 2)
  expect_equal(dplyr::filter(
    cloexp_anomalies, .data$Quantification_type == "seqCount")$Found, 2)
  ...
  expect_equal(dplyr::filter(
    ...
    .data$Quantification_type == "fragmentEstimate")$Found, 3)
  expect_equal(dplyr::filter(
    proj1100_1_anomalies, .data$Quantification_type == "seqCount")$Found, 1)
  ...
  expect_equal(dplyr::filter(
    proj1100_1_anomalies,
    .data$Quantification_type == "fragmentEstimate")$Found, 0)
  expect_equal(dplyr::filter(
    proj1100_1_anomalies, .data$Quantification_type == "seqCount")$Found, 0)
  ...
  expect_equal(dplyr::filter(
    ...
    .data$Quantification_type == "fragmentEstimate")$Found, 0)
  expect_equal(dplyr::filter(
    proj1100_2_anomalies, .data$Quantification_type == "seqCount")$Found, 0)
})
...
#------------------------------------------------------------------------------#
smpl_dupl <- function() {
  tb <- tibble::tibble(Quantification_type = c("fragmentEstimate", "seqCount",
                                               "fragmentEstimate"),
...
test_that(".choose_duplicates_files_interactive modifies tbl according to user
          choices", {
          input7 <- tempfile()
          op <- options(ISAnalytics.connection = c(NA, NA, NA, NA, NA, NA,
                                                   input7))
          on.exit(options(op), add = TRUE, after = FALSE)
          input_value7 <- "1"
          ...
          invisible(capture_output({
            updated_tbl <- .choose_duplicates_files_interactive(dupl_quant,
                                                                smpl_dupl_t)
          }))
...
#### OTHER VARS ####
files_found_fs_ann <- .lookup_matrices(as_file_correct,
                                       quant_types, "annotated")
files_found_fs_notann <- .lookup_matrices(as_file_correct,
                                       quant_types, "not_annotated")
files_found_fs_err_ann <- .lookup_matrices(as_file_err,
                                           quant_types, "annotated")
files_found_fs_err_notann <- .lookup_matrices(as_file_err,
                                           quant_types, "not_annotated")
...
test_that(".manage_anomalies_interactive succeeds with no anomalies", {
  ...
  present <- purrr::map2_lgl(to_import$ProjectID,
                             to_import$concatenatePoolIDSeqRun,
                             function(x, y) {
                               temp <- to_import %>%
                                 dplyr::filter(.data$ProjectID == x,
                                               .data$concatenatePoolIDSeqRun ==
                                                 y)
...
test_that(".manage_anomalies_interactive succeeds with anomalies", {
  input7 <- tempfile()
  input8 <- tempfile()
  op <- options(ISAnalytics.connection = c(NA, NA, NA, NA, NA, NA, input7,
                                           input8))
  on.exit(options(op), add = TRUE, after = FALSE)
  input_value7 <- "1"
  input_value8 <- "y"
...
test_that("import_parallel_Vispa2Matrices_interactive stops if af is missing", {
  expect_error({
    import_parallel_Vispa2Matrices_interactive(root = "x",
                                               quantification_type =
                                                 quant_types,
...
test_that("import_parallel_Vispa2Matrices_interactive stops if af is not
          character or tibble", {
  expect_error({
    import_parallel_Vispa2Matrices_interactive(association_file = 1,
                                               root = "x",
                                               quantification_type =
                                                 quant_types,
                                               matrix_type = "annotated",
...
test_that("import_parallel_Vispa2Matrices_interactive stops if af is a character
  ...
  expect_error({
    import_parallel_Vispa2Matrices_interactive(association_file = c(path_af,
                                                                    path_af),
                                               root = "x",
                                               quantification_type =
                                                 quant_types,
...
test_that("import_parallel_Vispa2Matrices_interactive stops if af is a character
  ...
  expect_error({
    import_parallel_Vispa2Matrices_interactive(association_file = path_af,
                                               quantification_type =
                                                 quant_types,
                                               matrix_type = "annotated",
                                               workers = 2)
...
test_that("import_parallel_Vispa2Matrices_interactive succeeds for fs - path", {
  ...
  input4 <- tempfile()
  input6 <- tempfile()
  op1 <- options(ISAnalytics.connection = c(input1, NA, input3, input4, NA,
                                            input6, NA, NA))
  op2 <- options("viewer" = NULL)
  on.exit({
    options(op1)
    options(op2)}, add = TRUE, after = FALSE)
  input_value1 <- "1"
  input_value3 <- "y"
  input_value4 <- "1"
  input_value6 <- "y"
...
test_that(".pattern_matching matches multiple patterns", {
  p_matches <- .pattern_matching(c("file1", "file2", "file3test"), c("file",
                                                                 "1",
                                                                 "test"))
...
test_that(".update_as_option works for fs err - single pattern", {
  cloexp_pool6 <- (files_found_fs_err_ann %>%
    dplyr::filter(.data$ProjectID == "CLOEXP",
                  .data$concatenatePoolIDSeqRun == "POOL6-1"))$Files[[1]]
...
test_that(".update_as_option works for fs err - multiple patterns", {
  ...
  cloexp_matches <- .pattern_matching(cloexp_pool6$Files_found, c("NoMate",
                                                                  "test"))
  updated <- .update_as_option(cloexp_pool6, cloexp_matches, "ANY")
  expect_true(nrow(updated) == 2)
  expect_true(all(quant_types %in% updated$Quantification_type))
  ...
  proj1100_pool1_matches <- .pattern_matching(proj1100_pool1$Files_found,
                                              c("NoMate", "test"))
  ...
  proj1100_pool1_matches <- .pattern_matching(proj1100_pool1$Files_found,
                                              c("NoMate", "test"))
  updated <- .update_as_option(proj1100_pool1, proj1100_pool1_matches,
                               "OPTIONAL")
  expect_true(nrow(updated) == 3)
  expect_true(all(quant_types %in% updated$Quantification_type))
  expect_true(all(!is.na(updated$Files_found)))
  ...
  proj1100_pool2_matches <- .pattern_matching(proj1100_pool2$Files_found,
                                              c("NoMate", "test"))
  ...
  expect_true(all(quant_types %in% updated$Quantification_type))
  expect_true(all(is.na(updated$Files_found)))
  updated <- .update_as_option(proj1100_pool2, proj1100_pool2_matches,
                               "OPTIONAL")
  expect_true(nrow(updated) == 2)
  expect_true(all(quant_types %in% updated$Quantification_type))
  expect_true(all(!is.na(updated$Files_found)))
...
test_that(".manage_anomalies_auto correctly manages for fserr", {
  dupl_message <- paste("Duplicates found for some files: in automatic mode",
...
test_that("import_parallel_Vispa2Matrices_auto stops if af is missing", {
  expect_error({
    import_parallel_Vispa2Matrices_auto(root = "x",
                                               quantification_type =
                                                 quant_types,
                                               matrix_type = "annotated",
                                               workers = 2,
...
test_that("import_parallel_Vispa2Matrices_auto stops if af is not
          character or tibble", {
      expect_error({
        import_parallel_Vispa2Matrices_auto(association_file = 1,
                                                   root = "x",
                                                   quantification_type =
                                                     quant_types,
                                                   matrix_type = "annotated",

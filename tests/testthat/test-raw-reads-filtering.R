...
min_example <- tibble::tibble(CompleteAmplificationID = paste0("ID",
                                                               seq_len(1000)),
                              PoolID = paste0("POOL",
                                              sample(c(1,2,3,4), 1000,
                                                     ...
                                                     prob = c(0.1, 0.3,
                                                              0.3, 0.3))),
                              X = extraDistr::rtnorm(n = 1000, mean = 10000,
                                                     sd = 1, a = 1, b = 20000),
...
test_that(paste(func_name[2], "flags false ok"), {
  ...
  flag <- .flag_cond_outliers_pool_frag(proc = TRUE,
                                        tdist = 1.5,
                                        zscore = -2,
...
test_that(paste(func_name[3], "correct combined logic AND cross"), {
  flag <- .apply_flag_logic(x = TRUE, y = FALSE,
...
test_that(paste(func_name[4], "- single key norm test"), {
  res <- .process_pool_frag(min_example, key = "X", normality_test = TRUE,
  ...
  res <- .process_pool_frag(min_example, key = "Y", normality_test = TRUE,
                            normality_threshold = 0.05, log2 = FALSE)
...
test_that(paste(func_name[5], "- single key"), {
  ...
  ## ON X
  res <- .pool_frag_calc(min_example, key = "X", by_pool = TRUE,
  ...
  res <- .pool_frag_calc(min_example, key = "X", by_pool = FALSE,
                         min_samples_per_pool = samples_per_pool,
                         pool_col = "PoolID",
                         normality_test = FALSE,
                         normality_threshold = 0.05,
  ...
  res <- .pool_frag_calc(min_example, key = "Y", by_pool = TRUE,
                         min_samples_per_pool = samples_per_pool,
                         pool_col = "PoolID",
                         normality_test = FALSE,
                         normality_threshold = 0.05,
...
test_that(paste(func_name[7], "- filters ok"), {
  example <- tibble::tribble(~CompleteAmplificationID,
                             ~PoolID, ~X, ~Y, ~Z,

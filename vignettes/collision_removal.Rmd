...
You can process the collisions in two different ways, the recommended way is to pass the entire named list to the function, so it can realign other matrices automatically:

```{r}
# Pass the whole named list
matrices_processed <- remove_collisions(x = matrices, association_file = association_file, date_col = "SequencingDate", reads_ratio = 10)
```

If you have the verbose option active, a report file is produced at the end that shows the before and after for each subject (and some other details). This report is an html widget, so you can save it or export it for future reference if you need it.

The alternative way for using this function is to give only the sequence count matrix as input:

```{r}
# Pass the sequence count matrix only
matrices_processed_single <- remove_collisions(x = matrices$seqCount, association_file = association_file, date_col = "SequencingDate", reads_ratio = 10)
```
...
```{r}
seq_count_proc <- matrices_processed_single$seqCount
other_matrices <- matrices[!names(matrices) %in% "seqCount"] # Select only matrices that are not relative to sequence count
...
## Create the vignette
library("rmarkdown")
system.time(render("collision_removal.Rmd", "BiocStyle::html_document"))

## Extract the R code
library("knitr")
knit("collision_removal.Rmd", tangle = TRUE)
```

```{r createVignette2}
## Clean up
file.remove("collision_removal.bib")
```

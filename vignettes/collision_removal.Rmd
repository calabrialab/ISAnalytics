...
```{r import_af, R.options=options(ISAnalytics.widgets = FALSE)}
options(ISAnalytics.widgets = FALSE)
...
```{r importMatr, R.options=options(ISAnalytics.widgets = FALSE)}
...
You can process the collisions in two different ways, the recommended way is to 
pass the entire named list to the function, so it can realign other matrices 
automatically:

```{r removecoll1, R.options=options(ISAnalytics.widgets = FALSE)}
# Pass the whole named list
matrices_processed <- remove_collisions(x = matrices, 
...
The alternative way for using this function is to give only the sequence 
count matrix as input:

```{r removecoll2, R.options=options(ISAnalytics.widgets = FALSE)}
# Pass the sequence count matrix only
matrices_processed_single <- remove_collisions(x = matrices$seqCount, 
                                               association_file = 
                                                 association_file, 
                                               date_col = "SequencingDate", 
                                               reads_ratio = 10)
